# -*- coding: utf-8 -*-
"""
大跌抄底 V4 — V3 + sklearn 持仓期动态止损

sklearn 逐日分类器发现:
  持仓期浮亏>5%时最终胜率仅3%（165次快照）
  Day3浮亏>5%: 16笔全亏 0%胜率

V4 相比 V3 的1项改进:
  【1】动态止损: 持仓期浮亏>5%立即平仓（不等-8%硬止损）

使用方法：
  python script/run_strategy_market_backtest.py --strategy dip_hunting_v4 --max-positions 5
"""
...
META = {
    "id": "dip_hunting_v4",
    "name": "大跌抄底 V4",
    "description": "V4=V3+动态止损: 持仓期浮亏>5%立即平仓(sklearn发现胜率仅3%)。保留V3其余参数。预期胜率>48%回撤<15%。",
}
...
# -- 动态止损（V4 新增 #4）--
DYNAMIC_STOP_PCT = -5            # 持仓期浮亏>5%立即平仓（sklearn: 胜率仅3%）
...
def generate_signals(bars, market_stats=None):
    ...
    for i in range(min_idx, n):
        ...
        # 1. 动态止损（V4 新增）: 浮亏>5%立即平仓（sklearn: 最终胜率仅3%）
        if profit_pct <= DYNAMIC_STOP_PCT:
            reason = f"动态止损(浮亏{profit_pct:.1f}%>5%,sklearn胜率仅3%)"

        # 2. 硬止损
        elif profit_pct <= STOP_LOSS_PCT:
            reason = f"止损({profit_pct:.1f}%)"

        # 3. 时间止损（V3 新增）: 持仓≥12天仍亏>5%
