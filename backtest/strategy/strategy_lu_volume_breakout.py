# -*- coding: utf-8 -*-
"""
涨停回踩 + 量价齐升策略 (Limit-Up Pullback with Volume-Price Breakout)

核心理念：涨停是主力资金的"宣言"，但涨停次日追涨风险高。
  等涨停后回踩（距涨停≥2天），再用量价齐升条件筛选：
  —— 趋势没坏（5日/20日/40日回撤可控）
  —— 量能持续放大（5日量>20日量×1.3，量能加速）
  —— 涨时放量跌时缩量（涨跌量比≥1.1）
  —— 价格位置合理（站上MA10/MA20，接近20日高点但不过分）

策略流程：
  买入 = 20天内有过涨停 + 量价齐升 + 位置合理
  卖出 = 止损/止盈/趋势破坏/持仓到期

与 trend_following 共享卖出逻辑和 market_gate。
"""

from pathlib import Path
import sys

ROOT_DIR = Path(__file__).resolve().parents[2]
if str(ROOT_DIR) not in sys.path:
    sys.path.insert(0, str(ROOT_DIR))

from backtest.indicators import sma

META = {
    "id": "lu_volume_breakout",
    "name": "涨停回踩量价齐升",
    "description": "20天内有涨停(≥9.5%非一字板)+距涨停≥2天+量价齐升(5日涨>1%/站上MA20/量比>1.3/涨跌量比>1.1/近20日高点)。卖出：止损-12%/止盈+25%/暴跌-8%/14天到期。",
}

# ============ 可调参数 ============

# -- 涨停检测 --
LIMIT_UP_PCT = 9.5             # 涨停阈值（%）
LIMIT_UP_LOOKBACK = 20         # 回看天数
MIN_DAYS_AFTER_LU = 2          # 距最近涨停至少N天（排除次日追涨）

# -- 趋势过滤 --
PRICE_UP_5D_MIN = 0.01         # 5日涨至少1%
PRICE_UP_20D_MIN = -0.08       # 20日跌幅不超过8%
PRICE_DOWN_40D_MAX = -0.20     # 40日最大回撤不超过20%

# -- 位置过滤 --
CLOSE_ABOVE_MA20 = True        # 收盘必须站上MA20
PRICE_NEAR_20D_HIGH = 0.15     # 距20日高点 ≤ 15%
PRICE_ABOVE_MA20_MAX = 1.15    # 收盘不超过MA20的15%

# -- 量能过滤 --
VOL_RATIO_BUY = 1.3            # vol_5d / vol_20d ≥ 1.3
VOL_RATIO_MAX = 5.0            # 不超过5倍（异常爆量排除）
VOL_TREND_ACCEL = 1.0          # 近3日量 / 前3日量 ≥ 1.0（至少不萎缩）

# -- 量价配合 --
UP_VOL_RATIO = 1.1             # 上涨日平均量 / 下跌日平均量 ≥ 1.1
LOOKBACK_DAYS = 10             # 量价配合评估窗口

# -- 动量 --
MIN_CONSEC_UP = 1              # 至少1天上涨

# -- 卖出 --
STOP_LOSS_PCT = -12            # 硬止损：相对买入价跌超此值离场（%）
TAKE_PROFIT_PCT = 25           # 止盈（%）
MAX_HOLD_DAYS = 14             # 最大持仓天数
DAILY_CRASH_PCT = -8           # 单日暴跌超过此值离场（%）

# -- 市场择时门槛 --
MARKET_FEAR_BREADTH = 0.25     # 广度 < 此值 = 恐慌底，积极买入
MARKET_GREED_BREADTH = 0.75    # 广度 > 此值 = 过热顶，停止买入


# ============ market_gate ============

_CURRENT_GATE = {"state": "NEUTRAL", "max_hold": MAX_HOLD_DAYS}


def market_gate(date, market_stats):
    """市场择时：恐慌时扩大持仓窗口，过热时收紧。永远不挡买入。"""
    global _CURRENT_GATE
    s = market_stats.get(date, {})
    breadth = s.get("breadth", 0.5)

    if breadth < MARKET_FEAR_BREADTH:
        _CURRENT_GATE = {"state": "FEAR", "max_hold": 30}
        return {"allowed": True, "state": "FEAR", "max_hold": 30,
                "reason": f"恐慌底(广度{breadth:.2f})"}
    elif breadth > MARKET_GREED_BREADTH:
        _CURRENT_GATE = {"state": "GREED", "max_hold": 7}
        return {"allowed": True, "state": "GREED", "max_hold": 7,
                "reason": f"过热顶(广度{breadth:.2f})"}
    else:
        _CURRENT_GATE = {"state": "NEUTRAL", "max_hold": MAX_HOLD_DAYS}
        return {"allowed": True, "state": "NEUTRAL", "max_hold": MAX_HOLD_DAYS,
                "reason": f"正常(广度{breadth:.2f})"}


# ============ 辅助函数 ============

def _is_one_word_limit_up(high, low, chg_pct):
    """检测是否一字涨停（全日最高=最低，且涨幅≥涨停阈值）→ 买不到"""
    return high == low and chg_pct >= LIMIT_UP_PCT


def _find_recent_limit_up(daily_change, highs, lows, i, lookback=LIMIT_UP_LOOKBACK):
    """
    在 [i - lookback + 1, i] 区间内查找最近的涨停日。

    返回:
        (lu_index, lu_chg_pct) 或 (None, 0)
        lu_index: 涨停日索引，None 表示未找到
        lu_chg_pct: 涨停日涨幅（%）
    """
    best_idx = None
    best_chg = 0

    start = max(1, i - lookback + 1)
    for j in range(start, i + 1):
        chg_pct = daily_change[j] * 100
        if chg_pct >= LIMIT_UP_PCT:
            # 排除一字涨停（买不到的不算）
            if _is_one_word_limit_up(highs[j], lows[j], chg_pct):
                continue
            # 找最近的那个
            if best_idx is None or j > best_idx:
                best_idx = j
                best_chg = chg_pct

    if best_idx is not None:
        return best_idx, round(best_chg, 2)
    return None, 0


def _compute_price_volume_dynamics(closes, volumes, i, lookback=LOOKBACK_DAYS):
    """计算最近N天的量价配比（涨时放量 vs 跌时缩量）"""
    start = max(1, i - lookback + 1)
    up_vols, down_vols = [], []

    for j in range(start, i + 1):
        chg = closes[j] - closes[j - 1]
        vol = volumes[j] if volumes[j] else 0
        if chg > 0:
            up_vols.append(vol)
        elif chg < 0:
            down_vols.append(vol)

    up_avg = sum(up_vols) / len(up_vols) if up_vols else 0
    down_avg = sum(down_vols) / len(down_vols) if down_vols else 1
    up_vol_ratio = up_avg / down_avg if down_avg > 0 else 2.0

    # 连续上涨天数
    consecutive_up = 0
    for j in range(i, 0, -1):
        if closes[j] > closes[j - 1]:
            consecutive_up += 1
        else:
            break

    return {
        "up_vol_ratio": round(up_vol_ratio, 2),
        "consecutive_up": consecutive_up,
    }


# ============ 主策略 ============

def generate_signals(bars):
    """
    生成买卖信号。

    买入 = 20天内有涨停 + 距涨停≥2天 + 量价齐升 + 位置合理
    卖出 = 止损/止盈/单日暴跌/持仓到期
    """
    closes = [b["close"] for b in bars]
    highs = [b["high"] for b in bars]
    lows = [b["low"] for b in bars]
    volumes = [b.get("volume") or 0 for b in bars]
    n = len(closes)

    # 预计算每日涨跌幅
    daily_change = [0.0]
    for i in range(1, n):
        chg = closes[i] / closes[i - 1] - 1 if closes[i - 1] else 0
        daily_change.append(chg)

    # 预计算指标
    ma10 = sma(closes, 10)
    ma20 = sma(closes, 20)
    vol_ma5 = sma(volumes, 5)
    vol_ma20 = sma(volumes, 20)

    signals = []
    in_pos = False
    entry_price = None
    entry_index = None
    position_max_hold = MAX_HOLD_DAYS

    min_idx = 42  # 40日回撤需要40根 + buffer

    for i in range(min_idx, n):
        close = closes[i]

        # 检查指标有效性
        if ma10[i] is None or ma20[i] is None:
            continue
        if vol_ma5[i] is None or vol_ma20[i] is None or vol_ma20[i] == 0:
            continue

        vol_ratio = vol_ma5[i] / vol_ma20[i]
        daily_chg = daily_change[i]

        # ==================== 买入逻辑 ====================
        if not in_pos:
            # ---- 第一层：20天内有过涨停 ----
            lu_idx, lu_chg_pct = _find_recent_limit_up(
                daily_change, highs, lows, i, LIMIT_UP_LOOKBACK
            )
            if lu_idx is None:
                continue

            # 距最近涨停日 ≥ MIN_DAYS_AFTER_LU（给回踩留空间）
            days_since_lu = i - lu_idx
            if days_since_lu < MIN_DAYS_AFTER_LU:
                continue

            # ---- 第二层：量价齐升筛选 ----
            # 趋势过滤
            if closes[i - 5] <= 0:
                continue
            chg_5d = (close - closes[i - 5]) / closes[i - 5]
            if chg_5d < PRICE_UP_5D_MIN:
                continue

            chg_20d = (close - closes[i - 20]) / closes[i - 20] if closes[i - 20] > 0 else 0
            if chg_20d < PRICE_UP_20D_MIN:
                continue

            # 40日最大回撤
            lookback_40 = closes[i - 39:i + 1]
            peak_40 = max(lookback_40)
            trough_40 = min(lookback_40)
            max_dd_40 = (trough_40 - peak_40) / peak_40
            if max_dd_40 < PRICE_DOWN_40D_MAX:
                continue

            # 位置过滤
            if close <= ma10[i] or close <= ma20[i]:
                continue

            high_20 = max(highs[i - 19:i + 1])
            if (high_20 - close) / high_20 > PRICE_NEAR_20D_HIGH:
                continue

            if close > ma20[i] * PRICE_ABOVE_MA20_MAX:
                continue

            # 量能过滤
            if vol_ratio < VOL_RATIO_BUY or vol_ratio > VOL_RATIO_MAX:
                continue

            # 量能加速
            if i >= 6:
                recent_3 = sum(volumes[i - 2:i + 1]) / 3
                prior_3 = sum(volumes[i - 5:i - 2]) / 3
                if prior_3 > 0 and recent_3 / prior_3 < VOL_TREND_ACCEL:
                    continue

            # 量价配合
            dyn = _compute_price_volume_dynamics(closes, volumes, i)
            if dyn["up_vol_ratio"] < UP_VOL_RATIO:
                continue
            if dyn["consecutive_up"] < MIN_CONSEC_UP:
                continue

            # ==== 全部条件满足 → 买入（次日收盘价成交）====
            if i + 1 >= n:
                continue

            # 次日一字涨停无法买入
            next_high = highs[i + 1]
            next_low = lows[i + 1]
            next_close = closes[i + 1]
            next_chg = (next_close - close) / close if close > 0 else 0
            if next_high == next_low and next_chg >= 0.095:
                continue

            # 入场日确认：站上MA10/MA20
            if ma10[i + 1] is None or ma20[i + 1] is None:
                continue
            if next_close <= ma10[i + 1] or next_close <= ma20[i + 1]:
                continue

            in_pos = True
            entry_price = next_close
            entry_index = i + 1
            position_max_hold = _CURRENT_GATE.get("max_hold", MAX_HOLD_DAYS)
            position_state = _CURRENT_GATE.get("state", "NEUTRAL")

            signals.append({
                "date": bars[i + 1]["trade_date"],
                "action": "buy",
                "reason": (
                    f"涨停回踩({lu_chg_pct:.1f}%涨停|距{days_since_lu}天|"
                    f"5日{chg_5d*100:.1f}%|量比{vol_ratio:.1f}x|"
                    f"涨跌量比{dyn['up_vol_ratio']:.1f}x|"
                    f"连涨{dyn['consecutive_up']}天|市态{position_state})"
                ),
            })
            continue

        # ==================== 卖出逻辑 ====================
        if i == entry_index:
            continue

        hold_days = i - entry_index
        profit_pct = (close / entry_price - 1) * 100 if entry_price else 0

        reason = None

        if profit_pct <= STOP_LOSS_PCT:
            reason = f"止损({profit_pct:.1f}%)"
        elif profit_pct >= TAKE_PROFIT_PCT:
            reason = f"止盈({profit_pct:.1f}%)"
        elif daily_chg <= DAILY_CRASH_PCT / 100:
            reason = f"单日暴跌({daily_chg:.1%})"
        elif hold_days >= position_max_hold:
            reason = f"持仓{hold_days}天到期(市态{_CURRENT_GATE.get('state','?')})"

        if reason is None:
            continue

        signals.append({
            "date": bars[i]["trade_date"],
            "action": "sell",
            "reason": reason,
        })
        in_pos = False
        entry_price = None
        entry_index = None

    return signals


# ============ 独立运行 ============

if __name__ == "__main__":
    from backtest.strategy.runner import run_strategy_meta

    run_strategy_meta(META)
