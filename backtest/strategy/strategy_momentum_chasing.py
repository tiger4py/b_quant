# -*- coding: utf-8 -*-
"""
短期动量追涨策略 (Momentum Chasing)

核心理念：追近期强势股，搞一波就跑。
  1. 趋势过滤：5日/20日涨幅、40日最大回撤限制
  2. 量能确认：量比放大 + 量能加速 + 量价配合
  3. 位置过滤：站上MA10+MA20 + 接近20日高点

注意：所有条件只看过去5-40天量价，不判断趋势可持续性。
"""
...
META = {
    "id": "momentum_chasing",
    "name": "短期动量追涨",
    "description": "短期动量：5日涨>1%、20日趋势不差、40日回撤<20%、量比>1.3、涨跌量比>1.1、站上MA20、接近20日高点。次日收盘价买入（不含信号日收益）。卖出：止损-12%/止盈+25%/单日暴跌-8%/持仓14天到期。",
}
...
CRASH_GRACE_DAYS = 5         # 暴跌时若仍盈利，给5天观察期（洗盘容忍）
...
def generate_signals(bars):
    ...
    entry_price = None
    entry_index = None
    crash_grace_until = None  # 暴跌宽限期结束日
    ...
    for i in range(min_idx, n):
        ...
        if profit_pct <= STOP_LOSS_PCT:
            ...
            # 暴跌但仍在盈利 → 给5天观察期（可能是洗盘）
            if profit_pct > 0 and crash_grace_until is None:
                crash_grace_until = i + CRASH_GRACE_DAYS
            elif crash_grace_until is not None and i <= crash_grace_until:
                pass  # 宽限期内, 不卖出
            else:
                reason = f"单日暴跌({daily_chg:.1%})"
        elif crash_grace_until is not None and i > crash_grace_until:
            crash_grace_until = None  # 宽限期过, 重置

        # 宽限期内不触发到期退出
        if reason is None and hold_days >= MAX_HOLD_DAYS:
            if crash_grace_until is not None and i <= crash_grace_until:
                pass  # 宽限期内延长持有
            else:
