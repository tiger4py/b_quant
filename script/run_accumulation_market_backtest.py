...
from logic.backtest_cache import (
    ...
    compute_accumulation_market_result,
    save_backtest_cache,
)
...
def main():
    ...
    with Session() as sess:
        result = compute_accumulation_market_result(
            sess,
            days=1000,
            initial_cash=1000000,
            max_positions=5,
        )
        save_backtest_cache(sess, ACCUMULATION_MARKET_CACHE_KEY, result)
        summary = result["summary"]
        selection = result["selection"]
        print("Accumulation market backtest cached")
